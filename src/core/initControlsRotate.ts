...
function initControlsRotate(canvas: fabric.Canvas) {
  // 添加旋转控制响应区域 add rotation control response area
  ...
  // 渲染时，执行 after:render 事件，更新旋转控制图标 render, execute the after:render event, and update the rotation control icon
  ...
  // 旋转时，实时更新旋转控制图标 rotate, update the rotation control icon in real time
  canvas.on('object:rotating', (event) => {
    ...
    switch (event.transform?.corner) {
      ...
      default:
        break;
    } // 设置四角旋转光标 set the cursor of the four corners
  });
}
